...
def get_section_1():
    ...
    <p><strong>4) Primary Ground Truth DNA Storage Benchmark:</strong> Evaluated on the complete 5,386-base genome of Frederick Sanger's <strong>Bacteriophage &Phi;X174</strong> (NCBI <code>NC_001422.1</code>), GPC achieves <strong>0.00% strand loss up to 10 nt (20 bits)</strong> of helicase motor stall and reassembles unordered pools in 1.31 ms ($81.6\,\mu\text{s}$ per strand) with 100% bit-exact fidelity, whereas state-of-the-art schemes suffer 100.00% collapse.</p>
    ...
    <p><strong>6) Cross-Domain Extensions & Bare-Metal Edge Profiling:</strong> We establish secondary applicability to UAV telemetry under RF chirp jamming and low-power BCI neural telemetry, profiling the complete GPC codec on bare-metal ARM Cortex-M4 (1,842 bytes Flash, 4.2 KB SRAM, $552\,\mu\text{s}$ latency) with turnkey, zero-dependency replication scripts.</p>
