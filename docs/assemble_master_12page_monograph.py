...
<body>
  ...
  <div class="audit-banner">
    <strong>Audited Empirical Scale:</strong> Evaluated on Frederick Sanger's 1977 Bacteriophage &Phi;X174 5,386-base genome (NCBI <code>NC_001422.1</code>), 23,000 Monte Carlo trials across Oxford Nanopore R10.4 mixed noise, bare-metal ARM Cortex-M4 profiling, and cross-domain UAV/BCI telemetry stress suites. Open-source on GitHub (<code>github.com/RABNEER/GPC-Codec</code>).
  </div>

  <div class="abstract-box">
    <span class="abstract-title">Abstract</span>—Synthetic DNA data storage is emerging as the premier medium for ultra-dense, archival data preservation. However, retrieval fundamentally relies on sequencing unordered oligonucleotide pools through protein nanopores (e.g., Oxford Nanopore R10.4.1), where helicase motor slips cause burst deletions of 5 to 15 nucleotides. When a burst strikes the strand address header, the payload coordinate is lost, inducing catastrophic <strong>Strand Address Dropout</strong> where entire 150-nt payloads are discarded as unindexable orphans. Prior state-of-the-art burst deletion codes (Schoeny et al., IEEE 2017) and Varshamov-Tenengolts (VT) codes suffer complete collapse (100.0% strand loss) when burst deletions exceed 5 nt or under mixed background substitution noise (0.6% sub, 0.6% del, 0.4% ins). This paper presents <strong>Generalized Pāṭha Codes (GPC)</strong>, an asymptotically resilient, permutation-based synchronization coding framework inspired by ancient combinatorial recitation lattices (<em>Krama</em>, <em>Jaṭā</em>, and <em>Ghana-pāṭha</em>). By generalizing cyclic forward-reverse permutations into a parameterized family $\text{GPC}(k, d)$, GPC decouples coordinate synchronization from symbol entropy. While GPC features an inner code rate of $R = \frac{d}{k^2 + 2k - 2}$ ($R = 0.069$ for $k=3, d=1$), we resolve this code rate paradox by applying GPC <strong>strictly as an inner 29-nt Address Header</strong> on a 150-nt payload, incurring only <strong>16.20% true oligonucleotide overhead</strong> ($179\text{ nt} < 200\text{ nt}$ commercial synthesis limit). In empirical benchmarks on the authentic 5,386-base genome of <strong>Bacteriophage &Phi;X174</strong> (NCBI <code>NC_001422.1</code>), GPC achieves <strong>0.00% strand loss up to 10 nt (20 bits)</strong> of helicase motor stall and reassembles unordered pools in 1.31 ms ($81.6\,\mu\text{s}$ per strand) with 100% bit-exact fidelity, whereas baseline schemes collapse. Under the brutal Oxford Nanopore R10.4.1 mixed-noise testbed across 23,000 Monte Carlo trials, GPC bounds strand loss to $\le 9.30\%$, well within standard outer fountain code recovery limits ($15\text{--}20\%$), while baseline codes fail ($32\%\text{--}100\%$ loss). In accordance with scientific integrity, we rigorously report all empirical failure boundaries ($b > 32\text{ bits}$ / $16\text{ nt}$), bulk payload inefficiencies, and cross-domain generalizations to UAV flight telemetry and wireless BCI neural spike streaming.
