...
def get_section_8():
    ...
    <h3>E. In-Silico Sequence Stability & Nearest-Neighbor Modeling</h3>
    <p>To evaluate whether GPC-encoded oligonucleotides maintain physical stability during in-silico PCR amplification and storage, we modeled nearest-neighbor hybridization free energy $\Delta G^\circ$ using the published SantaLucia parameter tables [SantaLucia 1998]:</p>
...
def get_section_10():
    ...
    <p class="no-indent">Autonomous multi-robot systems, such as drone swarms executing perimeter reconnaissance or disaster response, operate in dynamic environments where collision avoidance depends strictly on high-frequency inter-agent telemetry [14]. To evaluate multi-agent swarm telemetry under electronic jamming, we constructed a high-fidelity numerical physics simulation platform modeling decentralized swarm flight kinematics in a 3D coordinate space under simulated RF packet dropouts.</p>

    <h3>A. 6-DOF Quadrotor Flight Dynamics Simulation</h3>
    <p>We modeled a decentralized swarm of $N_{\text{uav}} = 8$ quadrotors operating in a shared $100\text{ m} \times 100\text{ m} \times 30\text{ m}$ simulated airspace. The kinematic motion of each quadrotor $i \in \{1, \dots, N_{\text{uav}}\}$ is governed by standard 6-DOF translational and rotational equations of motion:</p>

    <div class="eq-box">
      $$\dot{\mathbf{p}}_i = \mathbf{v}_i, \quad m_i \dot{\mathbf{v}}_i = m_i \mathbf{g} + \mathbf{R}_i \mathbf{f}_i, \quad \mathbf{J}_i \dot{\boldsymbol{\omega}}_i = -\boldsymbol{\omega}_i \times \mathbf{J}_i \boldsymbol{\omega}_i + \boldsymbol{\tau}_i$$
      <span class="eq-num">(11)</span>
    </div>

    <p class="no-indent">where $\mathbf{p}_i \in \mathbb{R}^3$ denotes 3D position, $\mathbf{v}_i \in \mathbb{R}^3$ denotes linear velocity, $m_i = 1.25\text{ kg}$ is mass, $\mathbf{g} = [0, 0, -9.81]^T\text{ m/s}^2$ is gravitational acceleration, $\mathbf{R}_i$ is the body-to-world rotation matrix, $\boldsymbol{\omega}_i \in \mathbb{R}^3$ is body angular velocity, $\mathbf{J}_i = \text{diag}(0.014, 0.014, 0.025)\text{ kg}\cdot\text{m}^2$ is the moment of inertia tensor, $\mathbf{f}_i = [0, 0, T_i]^T$ is total rotor thrust, and $\boldsymbol{\tau}_i \in \mathbb{R}^3$ is control torque.</p>
